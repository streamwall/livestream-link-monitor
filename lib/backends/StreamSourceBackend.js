...
class StreamSourceBackend extends BaseBackend {
  ...
  async getKnownCities() {
    try {
      ...
      // Convert to the format expected by locationParser (array of [city, state] arrays)
      ...
      // Return the cache info (matches what GoogleSheetsBackend returns)
      return cacheInfo.cityMap || new Map();
    } catch (error) {
  ...
  async sync(type) {
    switch (type) {
      case 'urls':
        ...
        break;
      case 'ignoreLists':
        await this.getIgnoreLists();
        break;
      case 'cities':
        await this.getKnownCities();
        break;
      default:
