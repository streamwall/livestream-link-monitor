...
const knownCitiesCache = new Map(); // Map of normalized city name -> {city, state}
  ...
  for (const row of citiesData) {
    if (row && row.length >= 2) {
      ...
      if (city && state) {
        ...
        // WARNING: Cities with same name in different states will overwrite each other
        // The last one in the sheet wins (e.g., Columbus OH vs Columbus IN)
        // This is handled in parseLocation() by checking state when explicitly provided
...
function parseLocation(message) {
  ...
  // Strategy 1: Look for "city, state" or "city state" patterns FIRST
  // This ensures we match the correct city when state is specified
  ...
  while ((match = cityStatePattern.exec(message)) !== null) {
    ...
    if (stateAbbreviations[potentialState]) {
      ...
      // When a state is explicitly mentioned, verify it matches
      ...
      // If we found a city but state doesn't match, it's the wrong city (e.g., Columbus IN vs Columbus OH)
      // Continue searching instead of returning the wrong city
    }
  }

  // Strategy 2: Look for exact city matches only if no state was specified
  // This prevents matching the wrong Columbus when "Columbus, OH" is in the message
